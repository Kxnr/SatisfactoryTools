...
EXTRACTORS = [
    ProcessNode(name=f"extractor_{name}", input_materials=Materials(), output_materials=Materials(**{name: 10}), power_production=0, power_consumption=0)
    for name in RAW_MATERIALS
]

# paired producer from first quarter to second quarter
FIRST_ORDER_PROCESSES = [
    ProcessNode(name=f"first_order_{in_name}_to_{out_name}", input_materials=Materials(**{in_name: 15}), output_materials=Materials(**{out_name: 7}), power_production=0, power_consumption=0)
...
SECOND_ORDER_PROCESSES = [
    ProcessNode(name=f"second_order_{in_name}_to_{out_name}", input_materials=Materials(**{in_name: 21}), output_materials=Materials(**{out_name: 9}), power_production=0, power_consumption=0)
...
FINAL_PROCESSES = [
    ProcessNode(name=f"final_order_{first_in}_{second_in}_to_{out}", input_materials=Materials(**{first_in: 14, second_in: 12}), output_materials=Materials(**{out: 2}), power_production=0, power_consumption=0) for (first_in, second_in), out
...
FEEDBACK_PROCESSES = [
    ProcessNode(name=f"feedback_{in_name}_to_{out_name}", input_materials=Materials(**{in_name: 10}), output_materials=Materials(**{out_name: 5}), power_production=0, power_consumption=0)
...
SIDE_PRODUCT_PROCESSES = [
    ProcessNode(name=f"side_product_{first_in}_{second_in}_to_{first_out}_{second_out}", input_materials=Materials(**{first_in: 6, second_in: 8}), output_materials=Materials(**{first_out: 2, second_out: 3}), power_production=0, power_consumption=0) for (first_in, second_in), (first_out, second_out)
